...
class BullFighter:
    ...
    def filter_cells(self, available_position, bull_position):
        ...
        current_dist=self.cal_dist(self.x_position, bull_position)
        for nx, ny in available_position:
            if abs(self.x_position[0]-nx)<= 2 and abs(self.x_position[1]-ny)<=2:
                ...
                if self.cal_dist((nx, ny), self.x_position)<=current_dist:
                    filtered_cells.append((nx, ny))
                # if math.pow((self.x_position[0]-nx),2)+math.pow((self.x_position[1]-ny),2)<current_dist:
                #     filtered_cells.append((nx, ny))
                ...
                if nx == robot_position[0] and ny == robot_position[1]:
                    ...
                    if abs(robot_position[0] - nx) + abs(robot_position[1] - ny) <= current_dist:
                        if self.maze_bull.is_obstacle(nx, ny):
                            ...
                            possible_movement.append((nx, ny))
        for new_position in possible_movement:
            total_length.append(math.pow((new_position[0] - bull_next_position[0]), 2) + math.pow(
    ...
    def guiding_mode(self, bull_position):
        ...
        if Astar_bull_path[1].get_position()==self.position:
            ...
            cur_dist=1000
            bull_next_position=0
            for cell in Astar_bull_path[1:]:
                pos=cell.get_position()
                dist=math.pow((pos[0]-self.position[0]),2)+math.pow((pos[1]-self.position[1]),1)
                if dist>cur_dist:
                    break
                cur_dist=dist
        ...
        for index, position in enumerate(available_position):
            sorted_solution.append((index, np.mean(self.simulate_for_guide_mode(bull_position, position, bull_next_position))))
